...
<div class="max-w-6xl mx-auto px-6 py-10" x-data="adminDashboard()">
    ...
    <h2 class="text-brand-accent font-display font-black text-2xl uppercase mb-6 tracking-wide">Panel de Control Edukan2</h2>
    
    <!-- Admin Tabs -->
    <div class="flex gap-2 mb-8 border-b border-gray-800 pb-1 overflow-x-auto scrollbar-none">
        ...
        <button :class="activeTab === 'cursos' ? 'text-white border-b-2 border-brand-accent font-bold' : 'text-white/50 hover:text-white'" class="pb-3 px-4 text-xs sm:text-sm font-semibold whitespace-nowrap cursor-pointer transition-all" @click="activeTab = 'cursos'">Diseñar Cursos</button>
    ...
    <div x-show="activeTab === 'dashboard'">
        <div class="bg-brand-dark2 border border-gray-800 p-6 rounded-2xl mb-8 shadow-xl">
            <h3 class="text-white font-bold text-sm mb-4">Editor de Prueba Social</h3>
            <form action="{{ route('admin.update-hero-stats') }}" method="POST">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-4 gap-4 items-end">
                    <div class="flex flex-col gap-1.5">
                        ...
                        <input type="text" name="alumnos" value="{{ $proofSocial['alumnos'] }}" class="w-full bg-brand-dark border border-gray-800 rounded-lg px-4 py-2.5 text-sm text-white focus:border-brand-accent focus:outline-none">
                    </div>
                    <div class="flex flex-col gap-1.5">
                        <label class="text-xs font-bold text-brand-text-muted">Tasa de Éxito</label>
                        <input type="text" name="exito" value="{{ $proofSocial['exito'] }}" class="w-full bg-brand-dark border border-gray-800 rounded-lg px-4 py-2.5 text-sm text-white focus:border-brand-accent focus:outline-none">
                    </div>
                    <div class="flex flex-col gap-1.5">
                        <label class="text-xs font-bold text-brand-text-muted">Países Alcanzados</label>
                        <input type="text" name="paises" value="{{ $proofSocial['paises'] }}" class="w-full bg-brand-dark border border-gray-800 rounded-lg px-4 py-2.5 text-sm text-white focus:border-brand-accent focus:outline-none">
                    </div>
                    <button type="submit" class="bg-brand-accent hover:bg-brand-accent/90 text-white font-bold py-2.5 rounded-lg text-sm transition-all cursor-pointer h-[42px]">Publicar en la Web</button>
                </div>
            </form>
        </div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-6 gap-4 mb-8">
            <div class="bg-brand-dark2 border border-gray-800 p-5 rounded-2xl shadow-xl flex flex-col justify-between border-l-4 border-brand-accent">
                <div class="text-[10px] text-brand-text-muted font-bold uppercase tracking-wider">Tráfico Total</div>
                <div class="text-2xl font-black text-white mt-2 font-display">{{ $visitas }}</div>
            </div>
            <div class="bg-brand-dark2 border border-gray-800 p-5 rounded-2xl shadow-xl flex flex-col justify-between border-l-4 border-brand-accent">
                <div class="text-[10px] text-brand-text-muted font-bold uppercase tracking-wider">Conversión</div>
                <div class="text-2xl font-black text-white mt-2 font-display">{{ $tasaConversion }}%</div>
            </div>
            <div class="bg-brand-dark2 border border-gray-800 p-5 rounded-2xl shadow-xl flex flex-col justify-between border-l-4 border-brand-success">
            ...
            <div class="bg-brand-dark2 border border-gray-800 p-5 rounded-2xl shadow-xl flex flex-col justify-between border-l-4 border-red-500">
            ...
            <div class="bg-brand-dark2 border border-gray-800 p-5 rounded-2xl shadow-xl flex flex-col justify-between border-l-4 border-purple-500">
            ...
            <div class="bg-brand-dark2 border border-gray-800 p-5 rounded-2xl shadow-xl flex flex-col justify-between border-l-4 border-brand-gold">
        ...
        <!-- Tasa BCV Editor -->
        <div class="bg-brand-dark2 border border-gray-800 p-5 rounded-2xl flex flex-col sm:flex-row justify-between items-center gap-4 mb-8 shadow-md">
            ...
            <form action="{{ route('admin.update-bcv') }}" method="POST" class="flex gap-2">
                @csrf
                <input type="number" name="euroBCV" step="0.01" value="{{ $bcvRate }}" class="bg-brand-dark border border-gray-800 rounded-lg px-3 py-1.5 text-sm text-white focus:border-brand-accent focus:outline-none w-24">
                <button type="submit" class="bg-brand-accent hover:bg-brand-accent/90 text-white font-bold px-4 py-1.5 rounded-lg text-xs transition-all cursor-pointer">Actualizar Tasa</button>
            </form>
        </div>
    </div>
    ...
    <div x-show="activeTab === 'crm'" class="hidden" :class="activeTab === 'crm' ? '!block' : ''">
        <div class="bg-brand-dark2 border border-gray-800 p-6 rounded-2xl mb-6 shadow-xl">
            <h3 class="text-white font-bold text-sm mb-4">Filtros y Buscador CRM</h3>
            
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <input type="text" x-model="crmSearch" @input.debounce.300ms="loadCrmStudents()" placeholder="Buscar por Nombre, Email o WhatsApp..." class="w-full bg-brand-dark border border-gray-800 rounded-lg px-4 py-2.5 text-sm text-white focus:border-brand-accent focus:outline-none">
                
                <select x-model="crmFilterMembership" @change="loadCrmStudents()" class="w-full bg-brand-dark border border-gray-800 rounded-lg px-4 py-2.5 text-sm text-white focus:border-brand-accent focus:outline-none cursor-pointer">
                ...
                <select x-model="crmFilterStatus" @change="loadCrmStudents()" class="w-full bg-brand-dark border border-gray-800 rounded-lg px-4 py-2.5 text-sm text-white focus:border-brand-accent focus:outline-none cursor-pointer">
        ...
        <div class="flex flex-col gap-4">
            <template x-for="student in crmStudents" :key="student.id">
                <div class="p-5 bg-white/[0.01] border border-gray-800 rounded-2xl flex justify-between items-center gap-4 flex-wrap hover:bg-white/[0.02] transition-all">
                    <div>
                        <div class="flex items-center gap-2.5 flex-wrap">
                            <strong class="text-white text-base font-bold" x-text="student.name"></strong>
                            <span :class="student.last_login_at ? 'text-brand-success' : 'text-red-500'" class="text-[10px] font-bold uppercase tracking-wider" x-text="student.last_login_at ? 'Activo' : 'Inactivo'"></span>
                        ...
                        <div class="text-xs text-brand-text-muted mt-2 flex gap-4 flex-wrap font-semibold">
                            ...
                            <span x-text="'Descuento: ' + (student.global_discount || '0') + '%'"></span>
                        </div>
                    </div>
                    <div class="flex gap-2">
                        ...
                        <button class="bg-brand-accent hover:bg-brand-accent/90 text-white text-xs font-bold px-3 py-2 rounded-lg cursor-pointer transition-all" @click="openManageModal(student.id, student.name)">Gestionar</button>
                    </div>
                </div>
            </template>
    ...
    <div x-show="activeTab === 'pagos'" class="hidden" :class="activeTab === 'pagos' ? '!block' : ''">
        <div class="bg-brand-dark2 border border-gray-800 p-6 rounded-2xl shadow-xl">
            ...
            <div class="overflow-x-auto scrollbar-none">
                <table class="w-full text-left border-collapse text-white min-w-[800px] text-xs sm:text-sm">
                    <thead>
                        <tr class="text-brand-text-muted border-b border-gray-800 font-bold uppercase text-[10px] tracking-wider">
                    ...
                    <tbody>
                        <template x-for="pago in payments" :key="pago.id">
                            <tr class="border-b border-gray-800 hover:bg-white/[0.01]">
    ...
    <div x-show="activeTab === 'notificaciones'" class="hidden" :class="activeTab === 'notificaciones' ? '!block' : ''">
        <div class="bg-brand-dark2 border border-gray-800 p-6 rounded-2xl max-w-lg shadow-xl">
            ...
            <form @submit.prevent="sendNotification()" id="notif-dispatch-form" class="flex flex-col gap-4">
                <div class="flex flex-col gap-1.5">
                    ...
                    <select name="destinatario" x-model="notifRecipient" class="w-full bg-brand-dark border border-gray-800 rounded-lg px-4 py-2.5 text-sm text-white focus:border-brand-accent focus:outline-none cursor-pointer">
                ...
                <div x-show="notifRecipient === 'individual'" class="flex flex-col gap-1.5">
                    ...
                    <input type="text" name="user_id" placeholder="Pega el ID del usuario" class="w-full bg-brand-dark border border-gray-800 rounded-lg px-4 py-2.5 text-sm text-white focus:border-brand-accent focus:outline-none">
                </div>

                <div class="flex flex-col gap-1.5">
                    <label class="text-xs font-bold text-brand-text-muted">Tipo de Mensaje</label>
                    <select name="type" class="w-full bg-brand-dark border border-gray-800 rounded-lg px-4 py-2.5 text-sm text-white focus:border-brand-accent focus:outline-none cursor-pointer">
                        <option value="info">Información / Noticia</option>
                        <option value="success">Descuento / Logro</option>
                        <option value="warning">Recordatorio de Pago / Urgente</option>
                    </select>
                </div>

                <div class="flex flex-col gap-1.5">
                    <label class="text-xs font-bold text-brand-text-muted">Título del Mensaje</label>
                    <input type="text" name="title" placeholder="Ej: ¡Actualización de Clase!" class="w-full bg-brand-dark border border-gray-800 rounded-lg px-4 py-2.5 text-sm text-white focus:border-brand-accent focus:outline-none" required>
                </div>

                <div class="flex flex-col gap-1.5">
                    <label class="text-xs font-bold text-brand-text-muted">Contenido</label>
                    <textarea name="message" placeholder="Escribe tu mensaje aquí..." class="w-full h-24 bg-brand-dark border border-gray-800 rounded-lg p-3 text-white text-xs sm:text-sm focus:border-brand-accent focus:outline-none resize-none font-body" required></textarea>
                </div>
    ...
    <div class="fixed inset-0 bg-brand-dark/90 flex items-center justify-center z-[99999] transition-all duration-300" x-show="auditModalOpen" x-transition class="hidden" :class="auditModalOpen ? '!flex' : 'hidden'">
        <div class="bg-brand-dark2 border border-gray-800 w-full max-w-lg rounded-2xl p-6 sm:p-8 relative shadow-2xl mx-6" @click.outside="auditModalOpen = false">
            ...
            <div class="flex flex-col gap-3 max-h-72 overflow-y-auto">
                <template x-for="grade in auditGrades">
                    <div class="bg-white/[0.01] p-4 rounded-xl border border-gray-800 flex justify-between items-center">
                        <div>
    ...
    <div class="fixed inset-0 bg-brand-dark/90 flex items-center justify-center z-[99999] transition-all duration-300" x-show="manageModalOpen" x-transition class="hidden" :class="manageModalOpen ? '!flex' : 'hidden'">
        <div class="bg-brand-dark2 border border-gray-800 w-full max-w-md rounded-2xl p-6 sm:p-8 relative shadow-2xl mx-6" @click.outside="manageModalOpen = false">
            ...
            <h2 class="text-white font-display font-black text-lg">Gestionar Estudiante VIP</h2>
            <p class="text-brand-accent text-xs font-bold tracking-wider mb-6 mt-1" x-text="manageName"></p>

            <div class="flex flex-col gap-4">
                <div class="bg-white/[0.01] p-4 border border-gray-800 rounded-xl">
                    <h4 class="text-white text-xs font-bold mb-2">Regalar Acceso a Curso</h4>
                    <select x-model="giftCourseId" class="w-full bg-[#1a1a24] border border-gray-800 rounded-lg p-2 text-xs text-white focus:border-brand-accent focus:outline-none cursor-pointer mb-3">
                        <option value="">Selecciona un curso</option>
                        @foreach($courses as $c)
                            <option value="{{ $c->id }}">{{ $c->title }}</option>
                        @endforeach
                    </select>
                    <button @click="giftAccess()" class="w-full bg-brand-accent hover:bg-brand-accent/90 text-white font-bold py-2 rounded-lg text-xs transition-all cursor-pointer">Conceder Acceso Gratuito</button>
                </div>

                <div class="bg-white/[0.01] p-4 border border-gray-800 rounded-xl">
                    <h4 class="text-white text-xs font-bold mb-2">Asignar Descuento Exclusivo (%)</h4>
                    <input type="number" x-model="manageDiscount" placeholder="Ej: 50" class="w-full bg-[#1a1a24] border border-gray-800 rounded-lg p-2 text-xs text-white focus:border-brand-accent focus:outline-none mb-3">
                    <button @click="saveDiscount()" class="w-full border border-brand-accent text-brand-accent hover:bg-brand-accent/10 font-bold py-2 rounded-lg text-xs transition-all cursor-pointer">Activar Descuento Global</button>
                </div>
            </div>
        </div>
    </div>
